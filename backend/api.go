...
// handleTariffsImbalance is the HTTP handler for the /api/tariffs/imbalance endpoint.
// It returns the current real-time imbalance price.
func handleTariffsImbalance(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	price := GetCurrentImbalancePrice()
	response := map[string]interface{}{
		"imbalance_price": price,
	}
	json.NewEncoder(w).Encode(response)
}
...
func handleDevices(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	if r.Method == "GET" {
		rows, err := db.Query("SELECT id, name, template, host, port, modbus_id, username, password, has_grid_meter, has_battery, battery_capacity, inverter_rated_power_kw, charge_mode, battery_mode, ocpp_proxy_url, poll_interval, thermostat_normal_temp, thermostat_boost_temp FROM devices")
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
		...
		result, err := db.Exec("INSERT INTO devices (name, template, host, port, modbus_id, username, password, has_grid_meter, has_battery, battery_capacity, inverter_rated_power_kw, charge_mode, battery_mode, ocpp_proxy_url, poll_interval, thermostat_normal_temp, thermostat_boost_temp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", d.Name, d.Template, d.Host, d.Port, d.ModbusID, d.Username, d.Password, d.HasGridMeter, d.HasBattery, d.BatteryCapacity, d.InverterRatedPowerKw, d.ChargeMode, d.BatteryMode, d.OcppProxyUrl, d.PollInterval, d.ThermostatNormalTemp, d.ThermostatBoostTemp)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
...
func handleDevice(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "DELETE" {
		...
		_, err = db.Exec("UPDATE devices SET name = ?, template = ?, host = ?, port = ?, modbus_id = ?, username = ?, password = ?, has_grid_meter = ?, has_battery = ?, battery_capacity = ?, inverter_rated_power_kw = ?, charge_mode = ?, battery_mode = ?, ocpp_proxy_url = ?, poll_interval = ?, thermostat_normal_temp = ?, thermostat_boost_temp = ? WHERE id = ?",
			d.Name, d.Template, d.Host, d.Port, d.ModbusID, d.Username, d.Password, d.HasGridMeter, d.HasBattery, d.BatteryCapacity, d.InverterRatedPowerKw, d.ChargeMode, d.BatteryMode, d.OcppProxyUrl, d.PollInterval, d.ThermostatNormalTemp, d.ThermostatBoostTemp, id)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
