...
6. **Deployment:** Deployed via a `.deb` package that creates a `nems` user and `nems.service`. Also built as a custom Raspberry Pi OS image with `nginx` proxy, a minimal Wayland desktop (`wayfire`), and `rpi-connect` for remote screen sharing.
